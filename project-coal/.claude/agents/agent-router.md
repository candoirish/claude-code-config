...
Existing patterns:
- `feature/lce-competitor-benchmarks`
- `fix/lce-stage-transition`
- `feat/adatomy-clinic/brand-config`
- `feature/ads-analysis/pipeline-optimization`
- `fix/dashboard/brand-selector-filter`

**For parallel mode or when `@worktree` is specified**, create a dedicated worktree:
```bash
git worktree add ../coal-{slug} -b {type}/{scope}-{descriptive-slug}
```
