...
### Atoll
- [ ] Atoll issue status matches the pipeline state (`done` if PR merged, `in_progress` if PR just created — valid Atoll statuses: `backlog | todo | in_progress | done | cancelled`; report the current status regardless)
