...
Leave the status as `in_progress` during review — the Atoll board does not have an `in_review` column. Valid statuses: `backlog | todo | in_progress | done | cancelled`.

After verdict:
- APPROVED → leave as `in_progress` (pr-manager sets `done` after merge)
- BLOCKED → add a comment explaining the blocker; leave status as `in_progress`
