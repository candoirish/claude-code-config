...
Examples:
- `specs/feat-lce-add-competitor-benchmarks.spec.md`
- `specs/fix-adatomy-clinic-dispatch-timeout.spec.md`
- `specs/refactor-ads-analysis-streamline-pipeline.spec.md`

For sub-specs when splitting large work, append `--{sub-scope}`:
- `specs/feat-lce-add-competitor-benchmarks--backend-api.spec.md`
- `specs/feat-lce-add-competitor-benchmarks--ui-components.spec.md`
...
- **Category** — the tool or feature area: `LinkedIn Creative Engine`, `Adatomy Clinic`, `Ads Analysis`, `Brand Checker`, `Portal`, `Dashboard`, `Auth`, `Billing`, etc.
...
- `[LinkedIn Creative Engine][Fix] Fix explicit Stage 2 to Stage 3 handoff`
- `[LinkedIn Creative Engine][Feature] Add competitor creative benchmarks`
- `[Adatomy Clinic][Feature] Add brand-level configuration`
- `[Brand Checker][Fix] Close three analysis gaps in judgment pipeline`

**The agent does not have permission to create Atoll issues.** Instead, present the issue details for the user to create manually:

```
## Atoll Issue (create manually)

**Title:** [{Category}][{Type}] {Description}
**Description:** {1–3 sentences stating the problem, from the user's point of view}
...
After the user creates the issue and provides the issue ID, proceed to Step 5.

### Step 5 — Register in Pipeline Queue

After creating the spec and receiving the Atoll issue ID from the user, add an entry to `specs/_queue.json`:
