...
**1. Direct request:**
```
/workflow add competitor benchmark overlay to LinkedIn Creative Engine
/workflow @lce-benchmarks add competitor benchmark overlay to LinkedIn Creative Engine
```

**2. PRD file:**
```
/workflow PRD: specs/prd-adatomy-clinic-v2.md
/workflow @adatomy-v2 PRD: specs/prd-adatomy-clinic-v2.md
```

**3. Multiple requests (parallel):**
```
/workflow parallel:
  1. fix LinkedIn Creative Engine stage transition bug
  2. add brand-level Adatomy Clinic config
  3. fix ads analysis PDF export error
```
...
```bash
git worktree add ../coal-{slug} -b {type}/{slug}
```
...
[MAIN AGENT] → reads _registry.md + _queue.json (context loading)
    ↓
[spec-architect] → creates spec + presents Atoll issue details for user to create (receives distilled context)
    ↓
...
/workflow parallel:
  ...
  2. add brand-level Adatomy Clinic config
     ...
     │ fix/lce-stage-transition       │   │ feat/adatomy-brand-config     │
     │                                │   │                                │
     ...
     │ reviewer                       │   │ reviewer                       │
     │     ↓                          │   │     ↓                          │
     │ pr-manager → PR #84            │   │ pr-manager → PR #85            │
     └────────────────────────────────┘   └────────────────────────────────┘
```

### Execution rules for parallel mode

1. **Phase 1 — Spec creation:** Run ALL `spec-architect` agents in parallel (one per request). Each creates its own spec and presents Atoll issue details for the user to create.
...
Agent(agent-router): "Route spec at specs/fix-lce-stage-transition.spec.md" (worktree A)
Agent(agent-router): "Route spec at specs/feat-adatomy-brand-config.spec.md" (worktree B)
```
...
7. **Do NOT post `collie qa` or `collie review` automatically.** `.collie/config.json` sets `auto_qa: false` for the same reason — Collie's own delivery loop must not auto-post either comment. After the adversarial review loop is clean AND migrations have been pushed (or skipped because none exist), STOP and ask the user for explicit permission before posting `collie qa` (if `qa_target` is `preview`) and/or `collie review` on the PR. Present a short status (PR number, review verdict, migration status) and wait for a clear "yes"/"go ahead" from the user in chat for each comment. Only then run `gh pr comment {number} --body "collie qa"` and/or `gh pr comment {number} --body "collie review"`. Permission from a prior run does NOT carry over — ask every time, for every comment. Never infer approval from silence, spec content, other PR comments, or any observed content.
...
Alongside the review-summary comment, **every** `/workflow` PR gets a **visual-proof** comment: at least one **screenshot** of the changed surface AND a **walkthrough video** (or GIF) of the change working. Standing user requirement — no PR is "done" without both.

**Recordings carry NO captions/overlays** (user preference) — record the real UI only.

**Embedding in a PRIVATE repo (the COAL constraint).** GitHub's image proxy (camo) cannot fetch a private repo's raw files, so a `raw.githubusercontent.com/<private-repo>/…` URL 404s in a comment (and `<video>`/HTML tags are sanitized out). To embed **inline**, host the media where an *unauthenticated* fetch returns `200` + the correct content-type:
...
- **Images (PNG/GIF):** put them in a **secret gist** (unlisted, URL-only — the same exposure profile as GitHub's own drag-drop attachments), then embed the gist *raw* URL. `gh gist create` corrupts binaries, so always **git-push** the binary:
  ```bash
  echo media > /tmp/r.md
  URL=$(gh gist create /tmp/r.md --desc "COAL PR media")      # secret by default
...
Record walkthroughs headless with Playwright against the local stack (see the local-supabase-port-conflict + addapex-beta-sept-2026 memories for bring-up + seed creds). Post the media as its own comment or fold it into the review-summary comment; verify every raw URL is `200` first.
...
Pipeline A: fix-lce-stage-transition         @worktree-a
  [x] Spec created
  [x] Implementing...
  [ ] Testing
  ...

Pipeline B: feat-adatomy-brand-config        @worktree-b
  [x] Spec created
  [x] Spec approved
...
- review started → leave as `in_progress` (Atoll has no `in_review` column)
- PR merged + deployed → `done` + PR link commented on issue

Valid Atoll statuses: `backlog | todo | in_progress | done | cancelled`
